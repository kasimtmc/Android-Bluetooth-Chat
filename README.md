...
1. You usually don't need to open a server to connect to a companion device. These devices usually act as a server. You should choose what to do based on the device's features.
2. To start a conversation between two phones, one of the devices must act as a server. In this case, you can try to connect to surrounding devices as a client or open a server and wait for connection requests from other devices.
